...
for(ws in c("10k", "20k", "50k")){
  ...
  for(h in hs){
    ...
    #REML eGRM
    points(y=power_results$power_REML_eGRM, x=x_pos+offset, pch=1, col="dodgerblue")
    power <- power_results$power_REML_eGRM
    std <- sqrt(power*(1-power)/nreps)
    segments(x0=x_pos+offset, y0=power - std, y1=power + std, col="dodgerblue")
    ...
    # #REML eGRM
    # power_results <- read.table(paste("true_trees/window_based/", ws, "/h", h, "/power_results.txt", sep=''), header=TRUE)
    # points(y=power_results$power_REML_eGRM, x=x_pos+offset, pch=19, col="dodgerblue")
  ...
  legend(x="topleft", legend=c("local eGRM (true)", "local eGRM (Relate)", "local GRM", "GWAS"), pch=c(19, 1, 1, 1), col=c("dodgerblue","dodgerblue", "maroon2", "orange2"), bty='n')
}
...
window_sizes <- c("10k", "20k", "50k")

pdf("power_aH_erorBars_window_based_methods.pdf", height=5, width=15)
par(mfrow=c(1,3))
...
for(ws in window_sizes){
  ...
  legend(x="topleft", legend=c("10k", "20k", "50k"), pch=c(1, 1, 1), col=c("dodgerblue", "maroon2", "orange2"), bty='n')
}
