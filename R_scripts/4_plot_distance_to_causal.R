...
plot_one <- function(dir, max_y, max_x, LTY, MAIN, tree_type){
  ...
  plot(density(d_GWAS, na.rm = TRUE, bw="SJ"), col="orange2", 
  ...
  #add the rest relate
  lines(density(d_eGRM, na.rm = TRUE, bw="SJ"), col="dodgerblue", lty=LTY)
  lines(density(d_GRM, na.rm = TRUE, bw="SJ"), col="maroon2", lty=LTY)
  if(run_acat){
