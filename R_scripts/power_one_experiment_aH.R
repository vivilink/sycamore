plot_one <- function(method, rep, df_REML, df_HE, cutoff_REML, cutoff_HE, cutoff_GWAS, df_GWAS, out_dir, df_pheno){
  ...
  plot(df_REML$start, -log10(df_REML$p_values), main=paste(method, "REML rep", rep), xlab="position", ylab="-log10(p)", ylim=c(0,11))
  ...
  plot(df_HE$start, -log10(df_HE$p_values_HESD_Jackknife), main=paste(method, " HE rep", rep), xlab="position", ylab="-log10(p)")
  abline(v=causal_pos_pos, col = adjustcolor("red", alpha = 0.2))
  abline(v=causal_pos_neg, col = adjustcolor("blue", alpha = 0.2))
  abline(h=cutoff_HE, col="gray")

  #plot GWAS
  plot(df_GWAS$start, -log10(df_GWAS$p_value), main=paste("GWAS rep", rep), xlab="position", ylab="-log10(p)", ylim=c(0,11))
  abline(v=causal_pos_pos, col = adjustcolor("red", alpha = 0.2))
  abline(v=causal_pos_neg, col = adjustcolor("blue", alpha = 0.2))
...
power_one_experiment <- function(hsquared, REPS, folder, tree_type, region_type, window_size){
  ...
  cutoff_GWAS <- read.csv(paste("/data/ARGWAS/experiments_cutoff_N2K/diploid/GRM_eGRM/high_mut_trees/", tree_type, "/", region_type, "/", window_size, "/p_value_cutoffs_", "GWAS", ".csv", sep=''))$x
  ...
  for(i in 1:length(covariance_types)){
    ...
    cutoff_files[[covariance_types[i]]] <- read.csv(paste("/data/ARGWAS/experiments_cutoff_N2K/diploid/GRM_eGRM/high_mut_trees/", tree_type, "/", region_type, "/", window_size, "/p_value_cutoffs_", covariance_types[i], ".csv", sep=''))
    
    # if(tree_type %in% c("true_trees", "relate_trees")){
    #   cutoff <- read.csv(paste("/data/ARGWAS/experiments_cutoff_N2K/diploid/eGRM/", tree_type, "/p_value_cutoffs.csv", sep=''))
    # } else {
    #   cutoff_files[[covariance_types[i]]] <- read.csv(paste("/data/ARGWAS/experiments_cutoff_N2K/diploid/GRM_eGRM/high_mut_trees/", tree_type, "/p_value_cutoffs_", covariance_types[i], ".csv", sep=''))
    # }
  }
  result_matrices[["GWAS"]] <- m_results_GWAS
  
  for(rep in 1:reps){
    # if(rep == 175 | rep == 187 | rep == 190 | rep >= 192){
    #   next
    # }
    print(rep)
  ...
  print(paste("using cutoff", cutoff_files[["eGRM"]]$cutoff_p_REML, "for REML eGRM"))
  
  power_REML_GRM <- sum(result_matrices[["GRM"]][,"REML"] > cutoff_files[["GRM"]]$cutoff_p_REML, na.rm=TRUE) / reps
  print(paste("using cutoff", cutoff_files[["GRM"]]$cutoff_p_REML, "for REML GRM"))
  
  power_GWAS <- sum(result_matrices[["GWAS"]][,"GWAS"] > cutoff_GWAS, na.rm=TRUE) / reps
  print(paste("using cutoff", cutoff_GWAS, "for GWAS"))
