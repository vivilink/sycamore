...
colnames(results) <- c("p-value", "rsquared", "beta")

for(i in 2:51){
  fit <- lm(pheno$BMI ~ eigenvec[,i])
  results[i-1, 1] <- summary(fit)$coefficients[2,4]
  results[i-1, 2] <- summary(fit)$r.squared
  results[i-1, 3] <- summary(fit)$coefficients[2,1]
}

pdf("corr_eigenvec_BMI.pdf", width=7, height=3)
