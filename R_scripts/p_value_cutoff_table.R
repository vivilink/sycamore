...
t <- matrix(ncol=5)
colnames(t) <- c("tree type", "causal variants","testing window size","method", "p-value cutoff")
  ...
  for(variant_freq in c("rareVariant", "commonVariant")){
    for(region_type in c("window_based")){
      for(ws_testing in c("5k","10k")){ #, "20k", "50k" , "10k", ,"10k"
        ...
        t <- rbind(t, c(tree_type, variant_freq, ws_testing, "GWAS", round(cutoff_GWAS,4)))
        t <- rbind(t, c(tree_type, variant_freq, ws_testing, "ACAT-V", round(cutoff_acat,4)))
        t <- rbind(t, c(tree_type, variant_freq, ws_testing, "local eGRM", round(cutoff_eGRM,4)))
        t <- rbind(t, c(tree_type, variant_freq, ws_testing, "GRM", round(cutoff_GRM,4)))
...
t <- t[-1,]
t[which(t[,1] == "true_trees"),1] <- "true trees / all variants"
t[which(t[,1] == "relate_trees"),1] <- "Relate / typed variants"
