...
hs_all <- c(0.005, 0.01, 0.02, 0.05, 0.1) #, 0.07, 0.04, 0.0025, , 0.2 0.001, 0.0001, 0.0002, 0.0005, 
run_acat <- TRUE

for(tree_type in c("relate_trees", "true_trees")){ #, "relate_trees", true_trees
      ...
      for(ws_testing in c("5k")){ #, "20k", "50k" , "10k", ,"10k"
        for(ws_causal in  c("oneVariant")){
          ...
          for(hsquared in hs_all){
            ...
            print(paste(hsquared, file.exists(results_file)))
#            if(file.exists(results_file) == FALSE){
                pheno_file_dir <- paste("/data/ARGWAS/power_sims/stdpopsim/relate_trees/oneVariant/", variant_freq,"/eGRM_GRM/", region_type, "/tested", ws_testing, "/" ,sep="")
...
# source("/home/linkv/git/sycamore/R_scripts/2b_calculate_power_arg_needle.R")
source("/home/linkv/git/sycamore/R_scripts/2b_calculate_power_binary.R")
...
#read low freq true trees
for(propCausal in c(0.5)){ #0.1, 0.2, 0.5, 0.8
  for(tree_type in c("true_trees")){ #, "relate_trees", true_trees, relate_trees_allVariants, arg_needle_trees
    for(region_type in c("window_based")){
      for(ws_testing in c("5k")){ #, "20k", "50k" , "10k", ,"10k"
        for(typed_status in c("allowTyped")){
          for(ws_causal in  c("5k")){
            power_results_aH <- data.frame()
            for(hsquared in hs_all){
              folder=paste("/data/ARGWAS/power_sims/stdpopsim/binary/", tree_type, "/oneRegion/eGRM_GRM/", region_type, "/", ws_causal, "/tested", ws_testing, "/", typed_status, "/", "/propCausal", propCausal,"/",sep="")
              print(paste("analyzing folder", folder))
              if(run_acat){
                results_file <- paste(folder, "h", hsquared, "/power_results_acat.txt", sep='')
              } else {
