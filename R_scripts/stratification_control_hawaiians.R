...
plot_ps_eGRM <- function(directory, MAIN, LOG=FALSE, ending="_eGRM_trees_GCTA_REML_results.csv"){
	...
	if(LOG==FALSE){
		...
		plot_qq_log_REML(log(p_corrected), MAIN=paste(MAIN, n_reps_found))
	}
...
#title(ylab="local eGRM log", line=1)
plot_ps_eGRM("association_with_GRM_correction/CREBRF_test_region_withCorrection_", "LOCO GRM", LOG=TRUE)
plot_ps_eGRM("association_with_100PC/eGRM/CREBRF_test_region_withCorrection_", "100 PCs", LOG=TRUE)
