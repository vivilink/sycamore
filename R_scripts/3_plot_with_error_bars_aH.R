...
for(propCausal in c(0.2)){ #0.1,0.2,0.5,
  # pdf(paste("power_aH_erorBars_window_based_propCausal", propCausal, "_relateTreesAllVariants.pdf", sep=''), height=5, width=5)
  pdf(paste("power_aH_erorBars_window_based_propCausal", propCausal, "_arg_needle.pdf", sep=''), height=5, width=5)
  ...
  for(ws_testing in c("5k")){  # ,"10k"
    for(ws_causal in c("5k")){
      ...
      for(h in hs){
        ...
        # acat
        if(run_acat){
          points(y=power_results$power_acat, x=x_pos+2*offset, pch=1, col="black")
          power <- power_results$power_acat
          std <- sqrt(power*(1-power)/nreps)
          segments(x0=x_pos+2*offset, y0=power - std, y1=power + std, col="black")
        }
      ...
      if(run_acat){
        ...
        legend(x="bottomright", legend=c("true trees / all variants","Relate / typed variants", "arg-needle", "local eGRM", "local GRM", "GWAS", "ACAT-V"), pch=c(1,19, 17,  15, 15, 15, 15), col=c("gray","gray","gray", blu, pin, org, "black"), bty='n')
