...
class TpGWAS(TGWAS):
    ...
    def manhattan_plot_subset(self, variant_positions, subplot, index_min, index_max, *args):
        if index_min < 0 or index_max < 0:
            raise ValueError("data subset indeces for manhattan plotting must be positive")
        if index_max > len(self.p_values):
            raise ValueError("maximum data subset index cannot be larger than number of p-values")
            
        self.q_values = -np.log10(self.p_values)
        subplot.scatter(variant_positions[index_min:index_max], self.q_values[index_min:index_max], s=0.5, *args)
